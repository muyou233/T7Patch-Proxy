...
| `dev_tools=` | `1` = **开发工具模式**：采集界面英文文本到 `T7Patch\ui_dump.txt`（做词库 / 片段候选发现用；采集后跑 `.codebuddy\ref\dev_pipeline.py` 出清单）。默认关闭，**不放进菜单**，属开发工具。旧键名 `dump_ui_strings` 仍会被读取，下次重写配置时自动改成新名 |
