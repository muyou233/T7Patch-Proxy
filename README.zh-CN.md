...
- **性能优化**：消除 DLC 扫描引起的卡顿
- **旧版着色器编译器隔离**：启动时把引擎自带的 `d3dcompiler_46.dll` 改名成
  `d3dcompiler_46.dll.bak`，让引擎用不了它（关闭开关时改回原名）
...
- **线程安全修复**
  ...
  - 配置对象（昵称 / 房间密码 / 各开关）原本被 3 个线程无任何同步地读写；
    现已用一把互斥锁保护，引擎调用保持在锁外，字符串读取接口改为调用方提供缓冲区
  - 菜单里点保存原本只写进配置文件就结束了 —— 配置监视器看不见"本进程自己写的"那次改动，
    新值永远到不了引擎（改昵称必须重启）。现在保存会把推送显式交给后台线程（约 1 秒，
    通常是 100 毫秒左右）
...
3. 启动游戏，右上角出现 `Patch 3.08` 即安装成功；游戏目录会自动生成 `T7Patch\` 文件夹
4. **卸载**：删除该 `d3d11.dll` 即可。补丁另外只会碰一个游戏文件：旧版着色器编译器开关开着时，
   `d3dcompiler_46.dll` 被改名成 `d3dcompiler_46.dll.bak` —— 关掉该开关（或手动把文件名改回、
   用 Steam「验证游戏文件完整性」）即可还原
...
- **开关**：仅好友可加入（点击即生效）、隔离旧着色器编译器（文件改名，下次启动生效）、自动打开窗口
...
| `block_d3dcompiler46=` | `1` = 启动时把 `d3dcompiler_46.dll` 改名成 `.dll.bak`（默认开启，菜单里也可切换） |
| `menu_key=` | 呼出菜单的虚拟键码（默认 `45` = Insert） |
| `menu_auto_open=` | `1` = 识别到主菜单后约 1.5 秒自动打开菜单（默认 `0`） |
