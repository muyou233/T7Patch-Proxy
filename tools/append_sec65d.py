# -*- coding: utf-8 -*-
# Append the §65d block to today's daily log, refusing to double-append.
import io

SRC = r"E:\MyProject\T7Patch\T7Patch-Proxy-Private\tools\sec65d_append.txt"
DST = r"E:\MyProject\T7Patch\T7Patch-Proxy-Private\.codebuddy\memory\2026-09-16.md"
OUT = r"E:\MyProject\T7Patch\T7Patch-Proxy-Private\tools\append_sec65d.txt"

add = io.open(SRC, encoding="utf-8").read()
old = io.open(DST, encoding="utf-8").read()
marker = "## 65d. "
assert marker not in old, "已经有 §65d 了，别重复追加"
if not old.endswith("\n"):
    old += "\n"
new = old + add
